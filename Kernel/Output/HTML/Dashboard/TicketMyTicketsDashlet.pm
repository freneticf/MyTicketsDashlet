...
sub new {
    ...
    my $AdditionalPreferencesKey = 'UserDashboardTicketMyTicketsAdditionalFilter' . $Self->{Name};
...
sub Run {
    ...
    if ( !$TicketIDs ) {
        ...
        if ( !$Self->{Config}->{IsProcessWidget} || IsArrayRefWithData( $Self->{ProcessList} ) ) {
            ...
            my $Filter;
            if ( $Self->{Filter} eq 'Open' ) {
                $Filter = 'QueueIDs';
            }
            elsif ( $Self->{Filter} eq 'Pending' ) {
                $Filter = 'ServiceIDs';
    ...
    # show only my services if we have the filter
    if ( $TicketSearchSummary{Open} ) {
        $LayoutObject->Block(
            Name => 'ContentLargeTicketGenericFilterOpen',
            Data => {
                %Param,
                %{ $Self->{Config} },
                Name => $Self->{Name},
    ...
    # show only my services if we have the filter
    if ( $TicketSearchSummary{Reminder} ) {
        $LayoutObject->Block(
            Name => 'ContentLargeTicketGenericFilterReminder',
            Data => {
                %Param,
                %{ $Self->{Config} },
                Name => $Self->{Name},
    ...
    # show only my services if we have the filter
    if ( $TicketSearchSummary{Pending} ) {
        $LayoutObject->Block(
            Name => 'ContentLargeTicketGenericFilterPending',
            Data => {
                %Param,
                %{ $Self->{Config} },
                Name => $Self->{Name},
